      ...
      <div class="tabs_con">
        <div id="loginbox">
          ...
          <div v-if="!login_regise_sms" class="phone_login_msmCode">
            <input
              type="text"
              class="phone_login_sms"
              v-model="userInfo.code"
              placeholder="短信验证码"
            />
            <div class="code_panel">
              <a class="send_ticket" href="javascript:;" @click="getCode"
                >获取验证码</a
              >
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      login_regise_sms: false, //false短信登录 true密码登录
      qrCode: false,
      userInfo: {
        phone: "",
        password: "",
  ...
  methods: {
    ...
    //获取验证码
    async getCode() {
      const result = await this.$store.dispatch("getCode");
    ...
    async login() {
      ...
      let user = {
        ...this.userInfo,
        flag: this.login_regise_sms,
      };
      var phone = this.userInfo.phone;

      if (this.login_regise_sms) {
        if (
          this.userInfo.phone !== "" &&
          ...
          if (this.userInfo.phone.length === 11) {
            result = await this.$store.dispatch("login", user);
          } else {
            alert("手机号码有误，请重填");
          }
        }
...
.content {
  ...
  #custom_display_2 {
    ...
    .qrCode {
      ...
      right: 0px;
      top: 0px;
      background: url(https://account.xiaomi.com/static/res/fa9e760/account-static/respassport/acc-2014/img/icon_ercode.png)
        no-repeat;
    }
  }
